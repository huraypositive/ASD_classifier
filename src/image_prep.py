...
def detect_color_order_fundus(img, bg_threshold=10, rb_margin=8.0):
    """
    img: HxWx3, 현재 RGB라고 가정하고 들어온다고 보지 말고
         '채널 순서가 불명확한 3채널 이미지'라고 생각하고 판별
    return:
        need_bgr2rgb: True면 BGR->RGB 변환 필요
        info: debug info
    """

    if img.ndim != 3 or img.shape[2] != 3:
        return False, {"reason": "not_3channel"}

    # 현재 img를 그대로 RGB라고 가정했을 때의 gray
    # 원형 안저 영역만 대략 마스킹
    gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
    mask = (gray > bg_threshold)

    if mask.sum() < 100:
        return False, {"reason": "too_few_pixels"}

    r_mean = float(img[..., 0][mask].mean())
    g_mean = float(img[..., 1][mask].mean())
    b_mean = float(img[..., 2][mask].mean())

    # 정상 fundus는 대체로 R이 B보다 우세
    # B가 과하게 높으면 BGR로 잘못 해석되었을 가능성
    diff = b_mean - r_mean
    need_bgr2rgb = diff > rb_margin
    ...
    return need_bgr2rgb, info


def maybe_fix_bgr_rgb(img, bg_threshold=10, rb_margin=8.0, verbose=False):
    """
    자동 판별 후 필요 시 BGR->RGB 변환
    """
    if img.ndim != 3 or img.shape[2] != 3:
        return img

    need_bgr2rgb, info = detect_color_order_fundus(
        img,
    ...
    if verbose:
        print("[Color Detect]", info, "=> swap:", need_bgr2rgb)
